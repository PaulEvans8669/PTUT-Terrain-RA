...
namespace Assets.Scripts.lib
{
    class Chunk
    {
        ...
        private void GenerateChunk(GameObject terrainGameObject, int terrainSize)
        {
            ...
            ChunkGameObject.transform.localPosition = new Vector3(-64 + Id % terrainSize * MeshSize, 0, 32 -(Id / terrainSize) * MeshSize);
            ChunkGameObject.transform.rotation = new Quaternion(0, 0, 0, 0);
        ...
        {
            if (!hasVegetationAtLocaiton(localX, localZ))
            {
                int index = localZ * (MeshSize + 1) + localX;
                float height = ChunkGameObject.GetComponent<MeshFilter>().mesh.vertices[index].y;
                GameObject clone = GameObject.Instantiate(model) as GameObject;
                clone.transform.parent = ChunkGameObject.transform;
                clone.transform.localPosition = new Vector3(localX, height, localZ);
                clone.transform.localRotation = new Quaternion(0, 0, 0, 0);
                clone.name = "vegetation";
                clone.isStatic = true;
                GameObject.Destroy(clone.GetComponent<MeshCollider>());
                VegetationList.Add(clone);
                NeedsVegetationUpdate = true;
            }
        }

        private bool hasVegetationAtLocaiton(int localX, int localZ)
        {
            foreach(GameObject go in VegetationList)
            {
                if(go.transform.localPosition.x == localX && go.transform.localPosition.z == localZ)
                {
                    return true;
                }
            }
        ...
        {
            float x = ChunkGameObject.transform.localPosition.x;
            float z = ChunkGameObject.transform.localPosition.z;
        ...
        private void loadNature(IDbConnection dbConnection, int terrainId)
        {
            ...
            while (reader.Read())
            {
                ...
                string type = reader.GetString(5);
                    ...
                    if (!hasVegetationAtLocaiton((int)x, (int)y)) {
